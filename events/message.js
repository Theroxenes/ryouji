...
const link = /(ryouji\.(glitch).(me|com)).*/;

 if(message.content.match(link)) return message.reply('Obrigado por promover meu site ! <:mi_anime_sorriso:415568069549752321>')
	...
	if (message.channel.type === 'dm') {
		if (!cmd) return;
		if (cmd.conf.guildOnly) return message.channel.send("Este comando está desativado em **DM's**");
	}
	...
	if (message.channel.type !== 'dm') {	// Grab the settings for this server from the enmap
		...
		if (message.content.match(/(discord\.(gg|me|io)|(discordapp\.com|discord\.com)\/invite).*/) && guildSettings.inviteFilterEnabled === 'true') {
			...
			message.delete();
			message.reply('Links de Convite não são permitidos');
		}

		if (guildSettings.swearFilter === 'true' && guildSettings.swearWords.some(word => message.content.includes(word))) {
			message.delete();
			message.reply('Palavras sujas não são toleradas em este servidor.');
		}
